...
export async function generateMetadata({ params, searchParams }) {
  ...
  const title = titleLine ? `${titleLine} — Log` : 'Check out this place on Log'
  const description = firstString(captionRaw).trim() || 'Open in the Log app to view this place.'
...
export default async function ShareBusinessBridgePage({ params, searchParams }) {
  const { id } = await params
  const sp = await searchParams
  const appStoreUrl =
...
(function () {
  ...
  var scheme = 'log://business/' + encodeURIComponent(id) + ${JSON.stringify(qs)};
  ...
  return (
    <>
      ...
      <Script id="log-share-business-bridge" strategy="afterInteractive" dangerouslySetInnerHTML={{ __html: inline }} />
    </>
  )
}
