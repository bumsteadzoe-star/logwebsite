import { isSupabasePublicObjectUrlAllowed } from './supabaseStorageOrigins'

/** Bump when OG pipeline changes so crawlers refetch (iMessage / Slack cache previews aggressively). */
export const OG_IMAGE_PIPELINE_VERSION = '8'
...
export function ogImageForMetadata(siteOrigin, raw) {
  ...
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL) return publicUrl
  if (!isSupabasePublicObjectUrlAllowed(publicUrl)) {
    return publicUrl
  }
